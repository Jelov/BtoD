void projectPbPbMC()
{
  ...
  TFile* outf = new TFile("bFeedDownPbPbMC_FineDPt.hist.root","recreate");
...
//  Float_t binsX[nBinX+1] = {2.,3.,4.,5.,6.,8.,10.,12.5,15.0,20.,25.,30.,40.,60.,100};
  ...
  for(int iCent=0; iCent<nCent; iCent++){
    ...
    for(int iPtWeight=0 ; iPtWeight<nPtWeight ; iPtWeight++){
      ...
      tMCP->Draw(Form("DsvpvDistance/DsvpvDisErr:DsvpvDistance*sin(Dalpha):Dpt>>hMCPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str() ),weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutPrompt&&cutCentralityBin));
      tMCNP->Draw(Form("DsvpvDistance/DsvpvDisErr:DsvpvDistance*sin(Dalpha):Dpt>>hMCNPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str()),weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutNonPrompt&&cutCentralityBin));
      tMCP->Draw(Form("DsvpvDistance*sin(Dalpha):Dmass:Dpt>>hPtMD0DcaMCPSignal_%s%s",  iCentStr.c_str(), iPtWeightStr.c_str()),cutpt&&cutPbPb&&cutmcSignal&&cutPrompt&&cutCentralityBin);
      tMCP->Draw(Form("DsvpvDistance*sin(Dalpha):Dmass:Dpt>>hPtMD0DcaMCPSwapped_%s%s",  iCentStr.c_str(), iPtWeightStr.c_str() ),cutpt&&cutPbPb&&cutmcSwapped&&cutPrompt&&cutCentralityBin);
