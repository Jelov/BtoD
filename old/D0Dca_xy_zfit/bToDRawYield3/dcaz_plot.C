...
//  h_MCNP_DCAz->GetXaxis()->SetRangeUser(0,0.04);
	...
	TLegend *l_DCAz = new TLegend(0.65,0.65,0.88,0.88);
	l_DCAz->AddEntry(h_MCP_DCAz,"Prompt Dca Z","l");
  l_DCAz->AddEntry(h_MCNP_DCAz,"NonPrompt Dca Z","l");
  ...
  l_DCAz->Draw();
	...
	h_MCP_DCAzEff->SetBinContent(1,1-h_MCP_DCAz->GetBinContent(1));
	h_MCNP_DCAzEff->SetBinContent(1,1-h_MCNP_DCAz->GetBinContent(1));

	for (int iBinZ = 2; iBinZ<=40; iBinZ++){
//	h_MCP_DCAzEff->SetBinContent(iBinZ,1-h_MCP_DCAz->Integral(1,iBinZ));
  h_MCP_DCAzEff->SetBinContent(iBinZ,h_MCP_DCAzEff->GetBinContent(iBinZ-1)-h_MCP_DCAz->GetBinContent(iBinZ));

	// cout<<"check 4"<<endl;
//  h_MCNP_DCAzEff->SetBinContent(iBinZ,1-h_MCNP_DCAz->Integral(1,iBinZ));
  h_MCNP_DCAzEff->SetBinContent(iBinZ,h_MCNP_DCAzEff->GetBinContent(iBinZ-1)-h_MCNP_DCAz->GetBinContent(iBinZ));
	// cout<<"check 5"<<endl;
}
...
//	h_MCP_DCAzEff->Draw();
	...
	TLegend *l_DCAzEff = new TLegend(0.65,0.65,0.88,0.88);
	l_DCAzEff->AddEntry((TObject*)0,"eff for lower DcaZ cut","");
