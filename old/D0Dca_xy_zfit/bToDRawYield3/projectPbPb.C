void projectPbPb()
{
  ...
  TCut cutpt = "Dpt>20 && sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2))>0.003";
...
*/
  ...
  TH3D* hSideband = new TH3D("hSideband",";p_{T} (GeV/c};D^{0} DCA_xy (cm);D^{0} DCA_z (cm)",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
  TH3D* hData = new TH3D("hData",";p_{T} (GeV/c};D^{0} DCA_xy (cm);D^{0} DCA_z (cm)",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
  ...
  tData->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hSideband",cutpt&&cutTriggerPbPb&&cutPbPb&&cutCentralityBin&&cutSideband);
  tData->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hData",cutpt&&cutTriggerPbPb&&cutPbPb&&cutCentralityBin&&cutSignal);
  tData->Draw("DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dmass:Dpt>>hPtMD0Dca",cutpt&&cutTriggerPbPb&&cutPbPb&&cutCentralityBin);
