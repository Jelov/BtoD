...
*/
  ...
  tData->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hSideband",cutpt&&cutTriggerPbPbMB&&cutPbPbMB&&cutCentralityBin&&cutSideband);
  tData->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hData",cutpt&&cutTriggerPbPbMB&&cutPbPbMB&&cutCentralityBin&&cutSignal);
