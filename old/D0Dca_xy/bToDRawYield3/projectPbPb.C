void projectPbPb()
{
  ...
  {  binsY[i] = binsY[i-1]+firstBinYWidth*pow(binYWidthRatio,i-1);
	cout<<std::setprecision(9)<<binsY[nBinY]<<", ";
	}
  ...
  TH3D* hSideband = new TH3D("hSideband",";p_{T} (GeV/c};D^{0} DCA (cm);flight distance significance",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
	TH3D* hData = new TH3D("hData",";p_{T} (GeV/c};D^{0} DCA (cm);D^{0} DCA_2d (cm)",nBinX,binsX,nBinY,binsY,nBinY,binsY);
  ...
  tData->Draw("(TMath::Sqrt(TMath::Power(DvtxX-PVx,2)+TMath::Power(DvtxY-PVy,2))* TMath::Sin( TMath::ACos( (TMath::Power((Dpt*TMath::Cos(Dphi)+(DvtxX-PVx)),2)+TMath::Power((Dpt*TMath::Sin(Dphi)+(DvtxY-PVy)),2)- TMath::Power(Dpt*TMath::Cos(Dphi),2)-TMath::Power(Dpt*TMath::Sin(Dphi),2)- TMath::Power(DvtxX-PVx,2) -TMath::Power(DvtxY-PVy,2)) / (2* TMath::Sqrt( TMath::Power(Dpt*TMath::Cos(Dphi),2)+TMath::Power(Dpt*TMath::Sin(Dphi),2))* TMath::Sqrt( TMath::Power(DvtxX-PVx ,2)+TMath::Power(DvtxY-PVy ,2) ) ) )) ):DsvpvDistance*sin(Dalpha):Dpt>>hData",cutpt&&cutTriggerPbPb&&cutPbPb&&cutCentralityBin&&cutSignal);
