...
*/
  ...
  tMCP->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCPSignal",weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutPrompt));
  tMCNP->Draw("sqrt(pow(DsvpvDistance*sin(Dalpha),2)-pow(abs(DvtxX*sin(Dphi)-DvtxY*cos(Dphi)),2)):DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCNPSignal",weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutNonPrompt));
