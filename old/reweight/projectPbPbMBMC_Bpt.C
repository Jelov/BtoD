...
//	tMCP->AddFriend(tMCPSkim);
	...
	TFile* outf = new TFile("bFeedDownPbPbMBMC_Bpt.hist.root","recreate");
...
//			hMCPSignal[iCent][iPtWeight] = new TH3D(Form("hMCPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str() ),";p_{T} (GeV/c);D^{0} DCA (cm);B p_P{T} (GeV/c)",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
...
//			tMCP->Draw(Form("DgenBAncestorpt:DsvpvDistance*sin(Dalpha):Dpt>>hMCPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str() ),weightfunctionreco*(cutpt&&cutPbPbMB&&cutSignal&&cutmc&&cutPrompt&&cutCentralityBin));
			tMCNP->Draw(Form("DgenBAncestorpt:DsvpvDistance*sin(Dalpha):Dpt>>hMCNPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str()),weightfunctionreco*(cutpt&&cutPbPbMB&&cutSignal&&cutmc&&cutNonPrompt&&cutCentralityBin));
