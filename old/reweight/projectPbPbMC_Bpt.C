...
//  tMCP->AddFriend(tMCPSkim);
  ...
  TFile* outf = new TFile("bFeedDownPbPbMC_Bpt.hist.root","recreate");
...
//      hMCPSignal[iCent][iPtWeight] = new TH3D(Form("hMCPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str() ),";p_{T} (GeV/c);D^{0} DCA (cm);flight distance significance",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
      hMCNPSignal[iCent][iPtWeight] = new TH3D(Form("hMCNPSignal_%s%s",  iCentStr.c_str(), iPtWeightStr.c_str() ),";p_{T} (GeV/c);D^{0} DCA (cm);B p_{T} (GeV/c)",nBinX,binsX,nBinY,binsY,nBinZ,binsZ);
...
//      tMCP->Draw(Form("DsvpvDistance/DsvpvDisErr:DsvpvDistance*sin(Dalpha):Dpt>>hMCPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str() ),weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutPrompt&&cutCentralityBin));
      tMCNP->Draw(Form("DgenBAncestorpt:DsvpvDistance*sin(Dalpha):Dpt>>hMCNPSignal_%s%s", iCentStr.c_str(), iPtWeightStr.c_str()),weightfunctionreco*(cutpt&&cutPbPb&&cutSignal&&cutmc&&cutNonPrompt&&cutCentralityBin));
