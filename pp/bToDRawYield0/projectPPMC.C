void projectPPMC()
{
  ...
  TFile* outf = new TFile("bFeedDownPPMC.hist.root","recreate");
...
*/
  ...
  tMCP->Draw("DsvpvDistance/DsvpvDisErr:DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCPSignal",weightfunctionreco*(cutpt&&cutpp&&cutSignal&&cutmc&&cutPrompt));
  tMCNP->Draw("DsvpvDistance/DsvpvDisErr:DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCNPSignal",weightfunctionreco*(cutpt&&cutpp&&cutSignal&&cutmc&&cutNonPrompt));
  tMCP->Draw("DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dmass:Dpt>>hPtMD0DcaMCPSignal",cutpt&&cutpp&&cutmcSignal&&cutPrompt);
  tMCP->Draw("DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dmass:Dpt>>hPtMD0DcaMCPSwapped",cutpt&&cutpp&&cutmcSwapped&&cutPrompt);
