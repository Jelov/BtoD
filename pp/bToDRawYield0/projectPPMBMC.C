...
*/
  ...
  tMCP->Draw("DsvpvDistance/DsvpvDisErr:DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCPSignal",weightfunctionreco*(cutpt&&cutPPMb&&cutSignal&&cutmc&&cutPrompt));
  tMCNP->Draw("DsvpvDistance/DsvpvDisErr:DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dpt>>hMCNPSignal",weightfunctionreco*(cutpt&&cutPPMb&&cutSignal&&cutmc&&cutNonPrompt));
  tMCP->Draw("DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dmass:Dpt>>hPtMD0DcaMCPSignal",cutpt&&cutPPMb&&cutmcSignal&&cutPrompt);
  tMCP->Draw("DvtxX*sin(Dphi)-DvtxY*cos(Dphi):Dmass:Dpt>>hPtMD0DcaMCPSwapped",cutpt&&cutPPMb&&cutmcSwapped&&cutPrompt);
