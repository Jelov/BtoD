...
void bToDRawYield()
{
	...
	for(int i=1; i<=nPtBins; i++)
	  //  for(int i=2; i<=2; i++)
	  {
		...
		TF1* background = new TF1("fBackground","[0]*exp([1]*(x-1.7))");
...
TF1* fitMass(TH1D* hData, TH1D* hMCSignal, TH1D* hMCSwapped)
{
	...
	TF1* f = new TF1("fMass","[0]*([7]*([9]*Gaus(x,[1],[2]*(1+[11]))/(sqrt(2*3.1415927)*[2]*(1+[11]))+(1-[9])*Gaus(x,[1],[10]*(1+[11]))/(sqrt(2*3.1415927)*[10]*(1+[11])))+(1-[7])*Gaus(x,[1],[8]*(1+[11]))/(sqrt(2*3.1415927)*[8]*(1+[11])))+[3]*exp([4]*(x-1.7))", 1.7, 2.0);
	...
	f->SetParameter(3, hData->GetBinContent(1));
	f->SetParameter(4, (hData->GetBinContent(60)/hData->GetBinContent(1)-1)/0.3);
	...
	TF1* massSwap = new TF1("fSwap","[0]*(1-[2])*Gaus(x,[1],[3]*(1+[4]))/(sqrt(2*3.1415927)*[3]*(1+[4]))");
