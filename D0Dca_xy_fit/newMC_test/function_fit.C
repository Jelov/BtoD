...
void SetHistStyle(TH1* h,int color = 1, int markerstyle =20, int linestyle=1){
...
void SetTexStyle(TLatex* tex,int font=42, double size=0.035, int width = 2){

    tex->SetNDC();
    tex->SetTextFont(font);
    tex->SetTextSize(size);
    tex->SetLineWidth(width);
...
void function_fit(){
  ...
  ofstream myfile;
  myfile.open("plots_functionfit/systematicTable.txt");


  TLatex* texCms = new TLatex(0.18,0.93, "#scale[1.25]{CMS} Preliminary");
  texCms->SetNDC();
  texCms->SetTextAlign(12);
  texCms->SetTextSize(0.03);
...
//  TH1D *h_DcaData = (TH1D*)fDcaData->Get("D0DcaDatafitOut_pt5");
  ...
  TH1D *h_PromptYield_fixP = new TH1D("h_PromptYield_fixP","h_PromptYield_fixP",nPtBins,ptBins);
  TH1D *h_NonPromptYield_fixP = new TH1D("h_NonPromptYield_fixP","h_NonPromptYield_fixP",nPtBins,ptBins);
  TH1D *h_NonPromptFraction_fixP= new TH1D("h_NonPromptFraction_fixP","h_NonPromptFraction_fixP",nPtBins,ptBins);
	
  TH1D *h_PromptYield_fixNP = new TH1D("h_PromptYield_fixNP","h_PromptYield_fixNP",nPtBins,ptBins);
  TH1D *h_NonPromptYield_fixNP = new TH1D("h_NonPromptYield_fixNP","h_NonPromptYield_fixNP",nPtBins,ptBins);
  TH1D *h_NonPromptFraction_fixNP= new TH1D("h_NonPromptFraction_fixNP","h_NonPromptFraction_fixNP",nPtBins,ptBins);
	...
	TCanvas *c_data2[nPtBins];

	TCanvas *c_dataAll = new TCanvas("c_dataAll","c_dataAll",1000,1000);
	c_dataAll->Divide(3,3);

	TCanvas *c_dataFitClosure[nPtBins];
...
//		MCNP_frame->SetMaximum(max+0.2*(max-min));
    ...
    gPad->SetLogy();
		...
		MCNPaVal = power_a.getVal();
		MCNPnVal = power_n.getVal();
...
//    min = data_frame2->GetMinimum();
    ...
    c_data[iPtBins]->SaveAs(Form("plots_functionfit/fitdata_pt%dto%d.png",(int)ptLow,(int)ptHigh));


		// plot c_dataFitClosure
/*
		///////////// working on here ///////////////

    c_dataFitClosure[iPtBins] = new TCanvas(Form("c_dataFitClosure_pt%i",iPtBins),Form("c_dataFitClosure_pt%i",iPtBins),600,800);
    c_dataFitClosure[iPtBins]->Divide(1,2);

		c_dataFitClosure[iPtBins]->cd(1);
		gPad->SetLogy();
		data_frame2log->Draw();
		tex_datafloat->Draw();
		
		c_dataFitClosure[iPtBins]->cd(2);
		TH1D *DataOverFit = new TH1D("DataOverFit","DataOverFit",nBinY,binsY);
	
		RooRealVar dca_subR("dca_subR","dca_subR",-0.07,0.07);
		RooArgSet nset(dca_subR);
		dca_subR.setRange("signal",0.0,0.002);
		RooAbsReal* idata = MCDCAmix_pdf.createIntegral(dca_subR,NormSet(dca_subR),Range("signal")); 
		double idataValue= idata->getVal();
	
		double dataValueOri=h_DcaData[iPtBins]->GetBinContent(11); 
		double dataValue=h_Data.sum(&nset); 
		double fitValue=MCDCAmix_pdf.getVal(&nset);
		double dataOverFit=dataValue/fitValue;

		cout<<"\n\nvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv"<<endl;
		cout<<"\n\n"<<endl;

		cout<<"dataValueOri = "<<dataValueOri<<endl;
		cout<<"dataValue = "<<dataValue<<endl;
		cout<<"fitValue = "<<fitValue<<endl;
		cout<<"idataValue = "<<idataValue<<endl;
		cout<<"dataOverFit = "<<dataOverFit<<endl;	
...
*/
    ...
    // plot c_dataAll
    c_dataAll->cd(iPtBins+1);
		gPad->SetLogy();
		RooPlot *data_frameAll = x.frame();
		h_Data.plotOn(data_frameAll,Binning(bin_dcaxy));
		MCDCAmix_pdf.plotOn(data_frameAll,LineColor(1));
		MCDCAmix_pdf.plotOn(data_frameAll,Components(power_pdf),LineStyle(kDashed),LineColor(4));
		MCDCAmix_pdf.plotOn(data_frameAll,Components(MCP_pdf),LineStyle(kDashed),LineColor(2));
		max = data_frameAll->GetMaximum();
		data_frameAll->SetMaximum(10*max);
		data_frameAll->SetMinimum(0.5);
		data_frameAll->GetXaxis()->SetTitle("D^{0} DCA (cm)");
		data_frameAll->Draw();

		TH1D *htemp_data = new TH1D("htemp_data","htemp_data",100,0,10);
    TH1D *htemp_fit = new TH1D("htemp_fit","htemp_fit",100,0,10);
    TH1D *htemp_MCP = new TH1D("htemp_MCP","htemp_MCP",100,0,10);
    TH1D *htemp_MCNP = new TH1D("htemp_MCNP","htemp_MCNP",100,0,10);
			
		SetHistStyle(htemp_data, 1, 20);
		SetHistStyle(htemp_fit);
		SetHistStyle(htemp_MCP, 2,20,3);
		SetHistStyle(htemp_MCNP, 4,20,3);

		TLegend *le_dataAll = new TLegend(0.68,0.65,0.88,0.88);
		le_dataAll->SetBorderSize(0);
		le_dataAll->AddEntry(htemp_data,"Data","pl");
		le_dataAll->AddEntry(htemp_fit,"Fit","l");
		le_dataAll->AddEntry(htemp_MCP,"Prompt D^{0}","l");
		le_dataAll->AddEntry(htemp_MCNP,"B to D^{0}","l");
		le_dataAll->Draw();


    TLatex *tex_Datapt2= new TLatex(0.18,0.82,Form("%.1f<P_{T}<%.1f GeV/c",ptLow,ptHigh));
    tex_Datapt2->SetNDC();
    tex_Datapt2->SetTextFont(42);
    tex_Datapt2->SetTextSize(0.035);
    tex_Datapt2->SetLineWidth(2);
    tex_Datapt2->Draw();

		TLatex *tex_pNum= new TLatex(0.18,0.75,Form("Prompt D^{0} yield : %.0f #pm %.0f",(float)pNum.getVal(),(float)pNum.getError()));
		SetTexStyle(tex_pNum);
		tex_pNum->Draw();

    TLatex *tex_npNum= new TLatex(0.18,0.68,Form("B to D^{0} yield : %.0f #pm %.0f",(float)npNum.getVal(),(float)npNum.getError()));
    SetTexStyle(tex_npNum);
    tex_npNum->Draw();
	...
	// fit data, fix one ,float another


		c_data2[iPtBins] = new TCanvas(Form("c_data2_pt%i",iPtBins),Form("c_data2_pt%i",iPtBins),1200,800);
		c_data2[iPtBins]->Divide(3,2);

		c_data2[iPtBins]->cd(1); //plot MC prompt fit
		gPad->SetLogy();
		MCP_frame->SetMaximum(MCP_frame->GetMaximum()*10);
		MCP_frame->SetMinimum(1);
//    gauss.plotOn(MCP_frame,LineColor(2));
    ...
    tex_Datapt->Draw();
		...
		c_data2[iPtBins]->cd(4);	//plot MC Non prompt fit
		gPad->SetLogy();
...
/*
    ...
    tex_MCNPGaussF->Draw();
*/
		c_data2[iPtBins]->cd(2); //plot data fit , parameter fixed
...
//		MCDCAmix_pdf.fitTo(h_Data);
		...
		RooPlot* data_frame3 = x.frame();
		h_Data.plotOn(data_frame3,Binning(bin_dcaxy));

		MCDCAmix_pdf.plotOn(data_frame3,Components(RooArgSet(MCP_pdf,power_pdf)),LineColor(3));
		MCDCAmix_pdf.plotOn(data_frame3,Components(power_pdf),LineStyle(kDashed),LineColor(4));
    MCDCAmix_pdf.plotOn(data_frame3,Components(MCP_pdf),LineStyle(kDashed),LineColor(2));
		MCDCAmix_pdf.paramOn(data_frame3);

    max = data_frame3->GetMaximum();
    min = data_frame3->GetMinimum();
    data_frame3->SetMaximum(max+0.2*(max-min));
		// data_frame3->SetMinimum(0.1); // must after the plotOn
		data_frame3->Draw();	

    TLatex *tex_datafixP= new TLatex(0.18,0.82,"P fix; NP float");
    tex_datafixP->SetNDC();
    tex_datafixP->SetTextFont(42);
    tex_datafixP->SetTextSize(0.035);
    tex_datafixP->SetLineWidth(2);
    tex_datafixP->Draw();

		// fill in output
		h_PromptYield_fixP->SetBinContent(iPtBins+1,pNum.getVal());
		h_PromptYield_fixP->SetBinError(iPtBins+1,pNum.getError());
    h_NonPromptYield_fixP->SetBinContent(iPtBins+1,npNum.getVal());
    h_NonPromptYield_fixP->SetBinError(iPtBins+1,npNum.getError());

    h_NonPromptFraction_fixP->SetBinContent(iPtBins+1,npNum.getVal()/(pNum.getVal()+npNum.getVal()));
    h_NonPromptFraction_fixP->SetBinError(iPtBins+1,npNum.getError()/(pNum.getVal()+npNum.getVal()));

	
		c_data2[iPtBins]->cd(5);
		gPad->SetLogy();
		RooPlot* data_frame3log = x.frame();
    h_Data.plotOn(data_frame3log,Binning(bin_dcaxy));
    MCDCAmix_pdf.plotOn(data_frame3log,Components(RooArgSet(MCP_pdf,power_pdf)),LineColor(3));
    MCDCAmix_pdf.plotOn(data_frame3log,Components(power_pdf),LineStyle(kDashed),LineColor(4));
    MCDCAmix_pdf.plotOn(data_frame3log,Components(MCP_pdf),LineStyle(kDashed),LineColor(2));
//    MCDCAmix_pdf.paramOn(data_frame3log);
    max = data_frame3log->GetMaximum();
    min = data_frame3->GetMinimum();
    data_frame3log->SetMaximum(10*max);
    data_frame3log->SetMinimum(0.5); // must after the plotOn
    data_frame3log->Draw();
    tex_datafixP->Draw();
		...
		power_a.setVal(MCNPaVal);
		power_n.setVal(MCNPnVal);	
		power_a.setConstant(kTRUE);
		power_n.setConstant(kTRUE);

		MCDCAmix_pdf.fitTo(h_Data,Extended(kTRUE));		
    RooPlot* data_frame4 = x.frame();
    h_Data.plotOn(data_frame4,Binning(bin_dcaxy));
    MCDCAmix_pdf.plotOn(data_frame4,Components(RooArgSet(MCP_pdf,power_pdf)),LineColor(3));
    MCDCAmix_pdf.plotOn(data_frame4,Components(power_pdf),LineStyle(kDashed),LineColor(4));
    MCDCAmix_pdf.plotOn(data_frame4,Components(MCP_pdf),LineStyle(kDashed),LineColor(2));
    MCDCAmix_pdf.paramOn(data_frame4);

	  max = data_frame4->GetMaximum();
    min = data_frame4->GetMinimum();
    data_frame4->SetMaximum(max+0.2*(max-min));
    data_frame4->Draw();

    TLatex *tex_datafixNP= new TLatex(0.18,0.82,"P float; NP fix");
    tex_datafixNP->SetNDC();
    tex_datafixNP->SetTextFont(42);
    tex_datafixNP->SetTextSize(0.035);
    tex_datafixNP->SetLineWidth(2);
    tex_datafixNP->Draw();


    // fill in output
    h_PromptYield_fixNP->SetBinContent(iPtBins+1,pNum.getVal());
    h_PromptYield_fixNP->SetBinError(iPtBins+1,pNum.getError());
    h_NonPromptYield_fixNP->SetBinContent(iPtBins+1,npNum.getVal());
    h_NonPromptYield_fixNP->SetBinError(iPtBins+1,npNum.getError());

    h_NonPromptFraction_fixNP->SetBinContent(iPtBins+1,npNum.getVal()/(pNum.getVal()+npNum.getVal()));
    h_NonPromptFraction_fixNP->SetBinError(iPtBins+1,npNum.getError()/(pNum.getVal()+npNum.getVal()));

		c_data2[iPtBins]->cd(6);
		gPad->SetLogy();
		RooPlot *data_frame4log = x.frame();
    h_Data.plotOn(data_frame4log,Binning(bin_dcaxy));
    MCDCAmix_pdf.plotOn(data_frame4log,Components(RooArgSet(MCP_pdf,power_pdf)),LineColor(3));
    MCDCAmix_pdf.plotOn(data_frame4log,Components(power_pdf),LineStyle(kDashed),LineColor(4));
    MCDCAmix_pdf.plotOn(data_frame4log,Components(MCP_pdf),LineStyle(kDashed),LineColor(2));
//    MCDCAmix_pdf.paramOn(data_frame4);
    max = data_frame4log->GetMaximum();
//    min = data_frame4->GetMinimum();
    data_frame4log->SetMaximum(10*max);
		data_frame4log->SetMinimum(0.5);
    data_frame4log->Draw();
    tex_datafixNP->Draw();
	

		c_data2[iPtBins]->SaveAs(Form("plots_functionfit/fitdata2_pt%dto%d.pdf",(int)ptLow,(int)ptHigh));
...
*/
  ...
  c_dataAll->SaveAs(Form("plots_functionfit/fitPbPbdataAll.pdf"));
  ...
  divideBinWidth(h_PromptYield_fixP);
  divideBinWidth(h_NonPromptYield_fixP);
  divideBinWidth(h_PromptYield_fixNP);
  divideBinWidth(h_NonPromptYield_fixNP);
...
//	h_NonPromptFraction_float->SetMaximum(1);;
  ...
  TCanvas *c_dataPtall2 = new TCanvas("c_dataPtall2","c_dataPtall2",1000,600);
  c_dataPtall2->Divide(3,2);
  c_dataPtall2->cd(1);
  gPad->SetLogy();
  h_PromptYield_fixP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_PromptYield_fixP->GetYaxis()->SetTitle("Prompt Yield (fixP)");
  h_PromptYield_fixP->Draw();
  c_dataPtall2->cd(2);
  gPad->SetLogy();
  h_NonPromptYield_fixP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_NonPromptYield_fixP->GetYaxis()->SetTitle("NonPrompt Yield (fixP)");
  h_NonPromptYield_fixP->Draw();
  c_dataPtall2->cd(3);
  h_NonPromptFraction_fixP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_NonPromptFraction_fixP->GetYaxis()->SetTitle("NonPrompt Fraction (fixP)");
  h_NonPromptFraction_fixP->Draw();
  c_dataPtall2->cd(4);
  gPad->SetLogy();
  h_PromptYield_fixNP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_PromptYield_fixNP->GetYaxis()->SetTitle("Prompt Yield (fixNP)");
  h_PromptYield_fixNP->Draw();
  c_dataPtall2->cd(5);
  gPad->SetLogy();
  h_NonPromptYield_fixNP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_NonPromptYield_fixNP->GetYaxis()->SetTitle("NonPrompt Yield (fixNP)");
  h_NonPromptYield_fixNP->Draw();
  c_dataPtall2->cd(6);
  h_NonPromptFraction_fixNP->GetXaxis()->SetTitle("D P_{T} (GeV/c)");
  h_NonPromptFraction_fixNP->GetYaxis()->SetTitle("NonPrompt Fraction (fixNP)");
//  h_NonPromptFraction_float->SetMaximum(1);;
  h_NonPromptFraction_fixNP->Draw();

  c_dataPtall2->SaveAs("plots_functionfit/fitPt_spectrum2.pdf");
  ...
  TH1D *h_PromptYield_diff_fixP = new TH1D("h_PromptYield_diff_fixP","h_PromptYield_diff_fixP",nPtBins,ptBins);
  TH1D *h_NonPromptYield_diff_fixP = new TH1D("h_PromptYield_diff_fixP","h_PromptYield_diff_fixP",nPtBins,ptBins);
  TH1D *h_PromptYield_diff_fixNP = new TH1D("h_PromptYield_diff_fixNP","h_PromptYield_diff_fixNP",nPtBins,ptBins);
  TH1D *h_NonPromptYield_diff_fixNP = new TH1D("h_PromptYield_diff_fixNP","h_PromptYield_diff_fixNP",nPtBins,ptBins);


  TH1D *h_PromptYield_RelErr2 = new TH1D("h_PromptYield_RelErr2","h_PromptYield_RelErr2",nPtBins,ptBins);
  TH1D *h_NonPromptYield_RelErr2 = new TH1D("h_NonPromptYield_RelErr2","h_NonPromptYield_RelErr2",nPtBins,ptBins);
	
  myfile<<"\n\n Start of Systematic Output \n"<<endl;

  myfile<<" pt , Prompt default, Prompt function, Percentage, Nonprompt default, NonPrompt function, Nonprompt Percentage"<<endl;
    ...
    h_PromptYield_diff_fixP->SetBinContent(iPtBins+1,h_PromptYield_fixP->GetBinContent(iPtBins+1)-h_PromptYield_tmp->GetBinContent(iPtBins+1));
    h_NonPromptYield_diff_fixP->SetBinContent(iPtBins+1,h_NonPromptYield_fixP->GetBinContent(iPtBins+1)-h_NonPromptYield_tmp->GetBinContent(iPtBins+1));

    h_PromptYield_diff_fixNP->SetBinContent(iPtBins+1,h_PromptYield_fixNP->GetBinContent(iPtBins+1)-h_PromptYield_tmp->GetBinContent(iPtBins+1));
    h_NonPromptYield_diff_fixNP->SetBinContent(iPtBins+1,h_NonPromptYield_fixNP->GetBinContent(iPtBins+1)-h_NonPromptYield_tmp->GetBinContent(iPtBins+1));

		Double_t max_diffP = TMath::Max((Double_t)abs(h_PromptYield_fixP->GetBinContent(iPtBins+1)-h_PromptYield_tmp->GetBinContent(iPtBins+1)),(Double_t)abs(h_PromptYield_fixNP->GetBinContent(iPtBins+1)-h_PromptYield_tmp->GetBinContent(iPtBins+1)));
		Double_t max_diffNP = TMath::Max((Double_t)abs(h_NonPromptYield_fixP->GetBinContent(iPtBins+1)-h_NonPromptYield_tmp->GetBinContent(iPtBins+1)),(Double_t)abs(h_NonPromptYield_fixNP->GetBinContent(iPtBins+1)-h_NonPromptYield_tmp->GetBinContent(iPtBins+1)));
    ...
    h_NonPromptYield_RelErr->SetBinContent(iPtBins+1,abs((h_NonPromptYield_float->GetBinContent(iPtBins+1)-h_NonPromptYield_tmp->GetBinContent(iPtBins+1))/h_NonPromptYield_tmp->GetBinContent(iPtBins+1)));
		...
		h_PromptYield_RelErr2->SetBinContent(iPtBins+1,abs(max_diffP/h_PromptYield_tmp->GetBinContent(iPtBins+1)));
		h_NonPromptYield_RelErr2->SetBinContent(iPtBins+1,abs(max_diffNP/h_NonPromptYield_tmp->GetBinContent(iPtBins+1)));

    myfile<<"Pt "<<ptLow<<"-"<<ptHigh<<" ,P "<<h_PromptYield_tmp->GetBinContent(iPtBins+1)<<" , "<<h_PromptYield_float->GetBinContent(iPtBins+1)<<" , "<<100*h_PromptYield_RelErr->GetBinContent(iPtBins+1)<<"%  , NP : "<<h_NonPromptYield_tmp->GetBinContent(iPtBins+1)<<" , "<<h_NonPromptYield_float->GetBinContent(iPtBins+1)<<" , "<<100*h_NonPromptYield_RelErr->GetBinContent(iPtBins+1)<<"%"<<endl;
...
//	h_NonPromptYield_RelErr->SetMaximum(1);
  ...
  TCanvas *c_yield_sys2 = new TCanvas("c_yield_sys2","c_yield_sys2",800,400);
  c_yield_sys2->Divide(2,1);

  c_yield_sys2->cd(1);
  h_PromptYield_RelErr2->Draw();
  TLatex *tex_promptY_RelErr2= new TLatex(0.18,0.82,"P Yield rel.Err fix & float ");
  tex_promptY_RelErr2->SetNDC();
  tex_promptY_RelErr2->SetTextFont(42);
  tex_promptY_RelErr2->SetTextSize(0.035);
  tex_promptY_RelErr2->SetLineWidth(2);
  tex_promptY_RelErr2->Draw();

	c_yield_sys2->cd(2);
	h_NonPromptYield_RelErr2->Draw();
  TLatex *tex_NonpromptY_RelErr2= new TLatex(0.18,0.82,"NP Yield rel.Err fix & float");
  tex_NonpromptY_RelErr2->SetNDC();
  tex_NonpromptY_RelErr2->SetTextFont(42);
  tex_NonpromptY_RelErr2->SetTextSize(0.035);
  tex_NonpromptY_RelErr2->SetLineWidth(2);
  tex_NonpromptY_RelErr2->Draw();
	

  c_yield_sys2->SaveAs("plots_functionfit/Yield_sys_FixnFloat.pdf");
  ...
  h_PromptYield_fixP->Write();
  h_NonPromptYield_fixP->Write();
  h_NonPromptFraction_fixP->Write();
  h_PromptYield_fixNP->Write();
  h_NonPromptYield_fixNP->Write();
  h_NonPromptFraction_fixNP->Write();

  h_PromptYield_diff_fixP->Write();
  h_NonPromptYield_diff_fixP->Write();
  h_PromptYield_diff_fixNP->Write();
  h_NonPromptYield_diff_fixNP->Write();
  h_PromptYield_RelErr2->Write();
  h_NonPromptYield_RelErr2->Write();
