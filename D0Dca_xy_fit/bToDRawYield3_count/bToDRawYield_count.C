...
TF1* fitMass(TH1D* hData, TH1D* hMCSignal, TH1D* hMCSwapped, double* CountResult);
void divideBinWidth(TH1* h);
void mutiplyBinWidth(TH1* h);


void bToDRawYield_count()
{
	...
	TH3D* hPtMD0DcaMCPSignal_1 = (TH3D*)hPtMD0DcaMCPSignal->Clone("hPtMD0DcaMCPSignal_1");
	hPtMD0DcaMCPSignal_1->Sumw2();
	TH3D* hPtMD0DcaMCPSwapped_1 =(TH3D*)hPtMD0DcaMCPSwapped->Clone("hPtMD0DcaMCPSwapped_1");
	hPtMD0DcaMCPSwapped_1->Sumw2();
	...
	TH1D *D0DcaDataOut_sideband[nPtBins];

	TH1D *h_NsignalDca[nPtBins];
	TH1D *h_NswappedDca[nPtBins];
	TH1D *h_NssratioDca[nPtBins];
	...
	/*	
			...
			Float_t binsYh[nBinYh+1] = [0.0,0.002,0.0046,0.0078,0.0118,0.0170,0.0236,0.0320,0.0428,0.0562,0.0734];


			TH1D *Dca_DataPos = new TH1D("Dca_DataPos","Dca_DataPos",nBinYh,binsYh);
			TH1D *Dca_DataNeg = new TH1D("Dca_DataNeg","Dca_DataNeg",nBinYh,binsYh);

			TH1D *Dca_MCPPos = new TH1D("Dca_MCPPos","Dca_MCPPos",nBinYh,binsYh);
			TH1D *Dca_MCPNeg = new TH1D("Dca_MCPNeg","Dca_MCPNeg",nBinYh,binsYh);
			TH1D *Dca_MCNPPos = new TH1D("Dca_MCNPPos","Dca_MCNPPos",nBinYh,binsYh);
...
*/
	...
	for(int i=2; i<=2; i++)
	{
		...
		TLatex* texPtY = new TLatex(0.19,0.82,Form("%.1f < p_{T} < %.1f GeV/c                   |y| < 1.0",ptLow,ptHigh));
		texPtY->SetNDC();
		texPtY->SetTextFont(42);
		texPtY->SetTextSize(0.045);
		...
		double CountResult[2]={0,0};

		TF1* fMass = fitMass(hMData, hMMCSignal, hMMCSwapped, CountResult);
		...
		hD0DcaData0->Draw();
		hD0DcaSideband->SetMaximum(10*hD0DcaSideband->GetMaximum());
		hD0DcaSideband->Draw("hsame");
		hD0DcaSideband0->SetLineStyle(2);
		hD0DcaSideband0->Draw("hsame");

		TLegend* leg1 = new TLegend(0.65,0.6,0.88,0.76,NULL,"brNDC");
		...
		leg1->AddEntry(hD0DcaSideband0,"side band","l");
		leg1->AddEntry((TObject*)0,"unscaled","");
		leg1->Draw("same");
		...
		fitMass(hMData, hMMCSignal, hMMCSwapped, CountResult);
		float yieldAll=fMass->GetParameter(0)*fMass->GetParameter(7)/hMData->GetBinWidth(1);

		cout<<"pts[i-1] = "<<pts[i-1]<<" ,yieldAll = "<<yieldAll<<endl;
		...
		for(int j=1; j<=20; j++)
		{
			...
			hMData_D0Dca->SetMaximum(5*hMData_D0Dca->GetMaximum());
			CountResult[0]=0;
			CountResult[1]=0;
			fMass = fitMass(hMData_D0Dca, hMMCSignal, hMMCSwapped, CountResult);
			...
			cout<<"\nCountResult[0] = "<<CountResult[0]<<" , yield = "<<yield<<endl;
			cout<<"CountResult[1] = "<<CountResult[1]<<" , yieldError = "<<yieldError<<"\n"<<endl;
			/*
				 hD0DcaDataFit->SetBinContent(j, yield);
				 hD0DcaDataFit->SetBinError(j, yieldError);
				 */
			// using count result
			hD0DcaDataFit->SetBinContent(j, CountResult[0]);
			hD0DcaDataFit->SetBinError(j, CountResult[1]);


			TLatex* texD0Dca = new TLatex(0.18,0.82,Form("D^{0} DCA_xy: %1.4f - %1.4f",D0DcaLow,D0DcaHigh));
			texD0Dca->SetNDC();
			texD0Dca->SetTextFont(42);
			texD0Dca->SetTextSize(0.045);
		...
		D0DcaDataOut_sideband[i-1]=(TH1D*)hD0DcaDataSubSideBand->Clone(Form("D0DcaDataOut_sideband_pt%i",i-1));
		mutiplyBinWidth(D0DcaDataOut_sideband[i-1]);


		double yieldAll2= D0DcaDataOut[i-1]->Integral();
		cout<<"yeildAll2 = "<<yieldAll2<<endl;
		...
		hD0DcaMCPSignal->SetMaximum(10*hD0DcaMCPSignal->GetMaximum());
		hD0DcaMCNPSignal->Draw("same");

		TLegend* leg2 = new TLegend(0.63,0.72,0.88,0.88,NULL,"brNDC");
		...
		hD0DcaData->SetMaximum(10*hD0DcaData->GetMaximum());
		hD0DcaData->Draw();
		int fitStatus = 1;
		...
		TLatex* texPrompt = new TLatex(0.61,0.74,Form("Prompt D^{0} yield : %.0f #pm %.0f",fMix->GetParameter(0),fMix->GetParError(0)));
		texPrompt->SetNDC();
		texPrompt->SetTextFont(42);
		texPrompt->SetTextSize(0.04);
		texPrompt->SetLineWidth(2);
		texPrompt->Draw();

		TLatex* texNonPrompt = new TLatex(0.61,0.66,Form("B to D^{0} yield : %.0f #pm %.0f",fMix->GetParameter(1),fMix->GetParError(1)));
		texNonPrompt->SetNDC();
		texNonPrompt->SetTextFont(42);
		texNonPrompt->SetTextSize(0.04);
		texNonPrompt->SetLineWidth(2);
		texNonPrompt->Draw();

		TLegend* leg4 = new TLegend(0.18,0.58,0.40,0.80);
...
/*
	...
	for (int i = 0; i<nPtBins;i++){
		...
		D0DcaDataOut_sideband[i]->Write();
	}

	for (int i =0; i<nPtBins;i++){
		h_NsignalDca[i]->Write();
		h_NswappedDca[i]->Write();
		h_NssratioDca[i]->Write();
	}
...
//  cout<<"count_data_sig = "<<count_data_sig<<" , fit_data_sig = "<<fit_data_sig<<endl;
  ...
  cout<<"maxhisto_dataSig = " <<maxhisto_dataSig<<" , maxhisto_sig = "<<maxhisto_sig<<endl;
	...
	TFitResultPtr fitResult = hData->Fit("fMass","L m S","",minhisto,maxhisto);
	double *cova= fitResult->GetCovarianceMatrix().GetMatrixArray();
	double cov00= fitResult->CovMatrix(0,0);

	cout<<"cov00 = "<<cov00<<" , cov01 ="<<fitResult->CovMatrix(0,1)<<endl;
	cout<<"cova = "<<*cova<<" , cova+1 = "<<*(cova+1)<<endl;

	double covmat[4][4];
	for(int icov =0; icov<4; icov++){
		for(int jcov =0; jcov<4; jcov++){
			covmat[icov][jcov]=fitResult->CovMatrix(3+icov,3+jcov);
		}
	}

	double *covmatArr=covmat[0];
	/*
		 cout<<"covmat[0][0]="<<covmat[0][0]<<" , *covmatArr ="<<*covmatArr<<endl;
		 cout<<"covmat[0][1]="<<covmat[0][1]<<" , *covmatArr + 1="<<*(covmatArr+1)<<endl;
		 cout<<"covmat[0][2]="<<covmat[0][2]<<" , *covmatArr + 2="<<*(covmatArr+2)<<endl;
		 cout<<"covmat[0][3]="<<covmat[0][3]<<" , *covmatArr + 3="<<*(covmatArr+3)<<endl;
		 cout<<"covmat[1][0]="<<covmat[1][0]<<" , *covmatArr + 4="<<*(covmatArr+4)<<endl;
		 */
	...
	cout<<"integralErr_sig = "<<integralErr_sig<<" ,integralVal = "<<integralVal_sig<<endl;
	cout<<"cand_integralVal = "<<cand_integralVal<<" ,cand_integralErr = "<<cand_integralErr<<endl;
	cout<<"binwidth = "<<binwidth<<endl;
	cout<<"\n************************\n\n"<<endl;

	CountResult[0]=(cand_integralVal-integralVal_sig/binwidth);
	//	CountResult[1]=sqrt( pow(cand_integralErr/cand_integralVal,2)+pow(integralErr_sig/integralVal_sig,2))*CountResult[0];
...
*/
	...
	TLegend* leg = new TLegend(0.65,0.53,0.85,0.88,NULL,"brNDC");
	leg->SetBorderSize(0);
